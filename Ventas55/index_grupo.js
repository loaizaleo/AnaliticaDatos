...
client.on('message', async msg => {
    try {
        ...
        if (chat.isGroup && gruposPermitidos.includes(chat.name)) {
            ...
            // ✅ Usar nombre del grupo para crear carpeta propia
            const nombreGrupo = chat.name.replace(/[^a-zA-Z0-9]/g, '_');

            // 📁 Crear carpetas personalizadas por grupo y fecha
            const logDir = path.join(__dirname, 'logs');
            const grupoDir = path.join(__dirname, 'media', nombreGrupo, fechaTexto);
            ...
            // 📸 Guardar archivos multimedia en carpeta del grupo
            if (msg.hasMedia) {
                const media = await msg.downloadMedia();
                const ext = media.mimetype.split('/')[1];
                const filename = `${grupoDir}/${Date.now()}.${ext}`;
